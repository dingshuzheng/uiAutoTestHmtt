...
class AppBase(Base):
    ...
    def app_base_right_wipe_left(self, loc_area, click_text):
        ...
        loc = By.XPATH, "//android.widget.HorizontalScrollView/*[contains(@text,'{}')]".format(click_text)  # //class/子级
        # 5.循环操作
        while True:
            # 1.获取当前屏幕页面结构
    ...
    def app_base_down_wipe_up(self, loc_area, click_text):
        ...
        # 组合频道元素配置信息
        # loc = By.XPATH, "//*[@class='android.widget.HorizontalScrollView']//*[contains(@text,'{}')]".format(click_text)
        loc = By.XPATH, "//*[@bounds='[0,520][1440,2280]']/*[contains(@text,'{}')]".format(click_text)  # //class/子级
        # 5.循环操作
        while True:
            # 1.获取当前屏幕页面结构
            page_source = self.driver.page_source
